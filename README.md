...
+ MLP networks on the GPU via SYCL (Intel oneAPI `icpx` compiler with
the CUDA backend). Models are described in JSON and trained on a CSV.
...
Requires Intel oneAPI (tested with 2025.1) and NVIDIA drivers.
...
For benchmarking, [bench.sh](bench.sh) rebuilds the binary and runs each
model in [models/](models/) 5 times, writing the timings to `bench_logs/`.
...
- [include/](include/) — layers (`linear_layer`, `conv1d_layer`,
  `pooling_layer`, `activation_layer`, `loss_layer`), SYCL matrix
  utilities in [include/math/](include/math/), JSON and CSV parsers.
- [models/](models/) — sample JSONs.
- [bench.sh](bench.sh) — benchmark script.
